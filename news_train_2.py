...
y = []
for i in dataset:
    x.append(i[2])  #sk 주식의 뉴스 데이터
    y.append(i[1]) #sk 주식의 등락 데이터

testx = []                      #데이터를 비율 맞추지 않고 트레인 데이터와 테스트 데이터로 날짜순 분류. Test 최근 데이터 20%, Train 나머지
testy = []
trainx = []
trainy = []
for i in range(len(x)):
    if i < (len(x) * 0.8):
        ...
        testx.append(x[i])
        testy.append(y[i])
x_train = Series(trainx)
x_test = Series(testx)
y_train = Series(trainy)
y_test = Series(testy)
...
history = model.fit(x_train_padded, y_train, epochs=5, batch_size=10, validation_split=0.2)
